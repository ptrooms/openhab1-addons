...
public class EdimaxBindingConfiguration implements BindingConfig {
	...
	public void parse(Item item, String bindingConfig)
		...
		// if (bindingConfig == null || "".equals(bindingConfig)) {		// 15okt25 ptro checkout why config is blank
    	if ( bindingConfig.equals(null) || "".equals(bindingConfig)) {
		...
		logger.debug("bindingConfig=" + bindingConfig + ", ConfigParts.length=" + configParts.length);
		macAddress = configParts[0];
		logger.debug("Edimax macaddress=" + macAddress);		// 15okt25 ptro checkout why config is blank

		if (configParts.length > 1) {
			password = configParts[1];
            logger.debug("Edimax password=" + password);		// 15okt25 ptro checkout why config is blank
		}
	...
	public String getPassword() {
		password = "edimax001";					// 15okt25 ptro force checkout why config is blank
		logger.debug("getPassword: return password=" + password + "." );	// 15okt25 ptro force checkout why config is blank
		return password;
	}
